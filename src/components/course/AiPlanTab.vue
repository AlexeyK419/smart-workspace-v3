  ...
  <div class="ai-plan-wrap card">
    <div class="ai-plan-header">
      <div>
        ...
        <div class="ai-plan-sub">Персонализированный план от GigaChat на основе материалов курса</div>
      </div>
      <button class="gen-btn" :class="{ loading: isLoading }" @click="generate" :disabled="isLoading">
        ...
        {{ isLoading ? 'GigaChat думает...' : 'Сгенерировать план' }}
      </button>
    </div>
    ...
    <div v-else-if="planText" class="plan-content">
      <div class="plan-toolbar">
        <div class="gigachat-badge">
          <span class="gc-dot"></span>
          Сгенерировано GigaChat · {{ settings.gigachat_model ?? 'GigaChat' }}
        </div>
...
// Model name for display — could come from /ai/models in real app
const settings = { gigachat_model: 'GigaChat' }
...
// Very light Markdown → HTML (no external lib needed)
